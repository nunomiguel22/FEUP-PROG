...
bool correctPosDir(string posDir, int x, int y, board b1) {
	...
	vector<vector <char>> boardVector = b1.returnWordTable();
	int startingX = posDir[1] - 96;
	int startingY = posDir[0] - 64;
	if (boardVector.at(startingY).at(startingX) == '#') 
		return false;
	return true;
}
